...
- Plain white pages, standard **20mm margin** on all sides (`.page-inner` padding). No colored border. This must stay white even though the upload page's outer chrome (`.upload-wrapper`/`.upload-card`, `app/static/styles.css`) uses a dark navy gradient theme — that dark theme is scoped to the upload screen only and must never bleed into `.page`/`.page-inner`, which is what both the browser preview and the Playwright PDF render.
...
- Bangladesh board/admission-exam reference tags (e.g. `[রা. বো. ১৪, ০২; সকল বোর্ড. ১৮]`, `[RUET. 07-08]`, or `[KUET: 04-05]`) are stripped from extracted content before rendering — Gemini's extraction includes these inline, sometimes as one combined bracket and sometimes as multiple adjacent brackets joined by `;`/`,`. Handled by `_strip_ref_tags` in `app/services/content_formatting.py` (a standalone module so `app/routers/pdf.py` stays a thin route layer): `_REF_MARKER` flags a bracket as a reference if it contains the Bengali abbreviation "বো" ("বোর্ড"/board) or an all-caps institute code followed by a year/number, with the code and year separated by `.`, `:`, or `-` (`RUET.`, `KUET:`, `DU-`, ...); `_BRACKET_RUN` then matches a whole run of adjacent bracketed tags (with their joining punctuation) so that removing one doesn't leave an orphaned `;`. A run is dropped only if at least one bracket in it matches `_REF_MARKER` — ordinary math brackets like `[a, b]` are left alone.
- The exercise heading (e.g. `অনুশীলনী-10.5`) is only permitted in the page header (`preview.html`'s `.doc-header-chapter`, driven by the "Enter chapter name" input) — extraction sometimes echoes it inline in the body text too, so `_EXERCISE_LABEL` in `app/services/content_formatting.py` strips that occurrence from the content. Groups left entirely blank after stripping (label was on its own line) are dropped in `format_content` rather than rendered as an empty `.content-group`, so they don't pick up the "between groups" 9mm top margin.
