...
- The **NumberNest logo** (`app/static/logo.svg`, inlined per `logo_svg` in `app/routers/pdf.py`) appears once, as a document header at the top of **page 1 only** — right-aligned, ~14mm tall, followed by a double rule (`#001A40`) separating it from the content. Not repeated on subsequent pages.
